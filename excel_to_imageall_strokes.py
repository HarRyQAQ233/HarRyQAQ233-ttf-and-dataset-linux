...
for z in lines:
    z = z.rstrip('\n')
    workBook = xlrd.open_workbook('/home/hanrufeng/datause/excel/' + str(z) + '.xls')
    h = z
    ...
    os.makedirs("/home/hanrufeng/datause/imageall-strokes/28/" + str(k), exist_ok=True)

    imgall = np.zeros((257, 257, 3), np.uint8)

    for x in range(0, k):
        imgall = imgall + images[x]
    imgall1 = cv2.resize(imgall, (28, 28), interpolation=cv2.INTER_AREA)  # 修改图片像素长度
    cv2.imwrite("/home/hanrufeng/datause/imageall-strokes/" + str(k) + "/" + str(h) + ".png", imgall1)  # 保存汉字完整图像
    print(z)
