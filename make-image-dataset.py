...
z = 0
for k in range(1, 24):
    f = open("/home/hanrufeng/datause/strokes-classify-number/" + str(k) + ".txt", "r")
    lines = f.readlines()
    aa = len(lines)

    for line in lines:
        x = line.rstrip('\n')
        img = cv2.imread("/home/hanrufeng/datause/imageall/28/" + str(x) + ".png", 1)
        for y in range(0, 10):
            ...
            dir_name = '/home/hanrufeng/dataset/28/'
            os.makedirs(dir_name + str(k), exist_ok=True)
            os.makedirs(dir_name + str(k) + "/" + str(x), exist_ok=True)
            cv2.imwrite(dir_name + str(k) + "/" + str(x) + "/" + str(x) + "_" + str(y + 1) + ".png", dst3)
        z = z + 1
        print(z)
